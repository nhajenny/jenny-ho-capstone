...
function FestivalDetails() {
            ...
            try {
                const [festivalRes, ticketsRes, hotelsRes, commentsRes] =
                    await Promise.all([
                        ...
                        axios.get(
                            `http://localhost:8080/api/tickets/festival/${id}`
                        ),
                        axios.get(
                            `http://localhost:8080/api/hotels/festival/${id}`
                        ),
                        axios.get(
    ...
    if (!festival)
        return <div className="festival-details__loading">Loading...</div>;

    return (
        <section className="festival-details">
            <div className="festival-details__header">
                <h1 className="festival-details__title">
                    {festival.festival_name}
                </h1>
                <p className="festival-details__location">
                    📍 {festival.location}
                </p>
            ...
            <div className="festival-details__content">
                <div className="festival-details__section">
                    <h2 className="festival-details__section-title">
                        Tickets 🎟️
                    </h2>
                    <div className="festival-details__tickets">
                        {tickets.length > 0 ? (
                            tickets.map((ticket) => (
                                <div
                                    key={ticket.id}
                                    className="festival-details__ticket-box"
                                >
                                    <p>
                                        <strong>Type:</strong>{" "}
                                        {ticket.ticket_type}
                            ...
                            <p className="festival-details__message">
                    ...
                    <div className="festival-details__hotels">
                        {hotels.length > 0 ? (
                            hotels.map((hotel) => (
                                <div
                                    key={hotel.id}
                                    className="festival-details__hotel-box"
                                >
                                    <p>
                                        <strong>Hotel:</strong>{" "}
                                        {hotel.hotel_name}
                                    ...
                                    <p>
                                        <strong>Cost:</strong>{" "}
                                        {hotel.cost_per_night} {hotel.currency}
                                    </p>
                                    <p>
                    ...
                    <div className="festival-details__comments">
                        {comments.length > 0 ? (
                            comments.map((comment) => (
                                <div
                                    key={comment.id}
                                    className="festival-details__comment-box"
                                >
                                    <p>
